    ...
    <el-form>
        <el-form-item>
            <el-radio v-model='radioValue' :value="1">
                {{ t('crontab.wildcard', { unit: t('crontab.unit.week'), chars: '[, - * ? / L #]' }) }}
            </el-radio>
        </el-form-item>
        ...
        <el-form-item>
            <el-radio v-model='radioValue' :value="4">
                {{ t('crontab.weekNumber') }}
                <el-input-number v-model='average01' :min="1" :max="4" /> {{ t('crontab.weekOn') }}
...
const weekList = computed(() => Array.from({ length: 7 }, (_, index) => ({
    key: index + 1,
    value: t(`crontab.weekdays.${index + 1}`)
})))
