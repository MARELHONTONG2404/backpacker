   ...
   <div class="app-container">
      ...
      <el-row :gutter="10" class="mb8">
         <el-col :span="1.5">
            ...
            >{{ t('system.auth.addUser') }}</el-button>
         </el-col>
         <el-col :span="1.5">
            <el-button
               type="danger"
            ...
            >{{ t('system.auth.batchCancelAuth') }}</el-button>
         </el-col>
         <el-col :span="1.5">
            <el-button 
               type="warning" 
      ...
      <el-table v-loading="loading" :data="userList" @selection-change="handleSelectionChange">
         ...
         <el-table-column :label="t('common.action')" align="center" class-name="small-padding fixed-width">
            <template #default="scope">
               <el-button link type="primary" icon="CircleClose" @click="cancelAuthUser(scope.row)" v-hasPermi="['system:role:remove']">{{ t('system.auth.cancelAuth') }}</el-button>
            </template>
         </el-table-column>
      </el-table>
...
function cancelAuthUser(row) {
  proxy.$modal.confirm(t('system.auth.confirmCancelAuth', { name: row.userName })).then(function () {
    return authUserCancel({ userId: row.userId, roleId: queryParams.roleId })
  }).then(() => {
    getList()
    proxy.$modal.msgSuccess(t('system.auth.cancelAuthSuccess'))
  }).catch(() => {})
}
...
function cancelAuthUserAll() {
  ...
  proxy.$modal.confirm(t('system.auth.confirmBatchCancel')).then(function () {
