  ...
  <el-drawer :model-value="visible" direction="rtl" size="700px" append-to-body @update:model-value="$emit('update:visible', $event)">
    ...
    <div class="drawer-wrap">
      ...
      <div v-else-if="!dataList.length" class="drawer-empty">
        ...
        <div>{{ t('system.dict.detailEmpty') }}</div>
      </div>
      ...
      <template v-else>
        <el-row :gutter="12" class="stat-row">
          <el-col :span="disabledCount > 0 ? 8 : 12">
            <div class="stat-card">
              ...
              <div class="stat-label">{{ t('system.dict.totalEntries') }}</div>
            </div>
          </el-col>
          <el-col :span="disabledCount > 0 ? 8 : 12">
            <div class="stat-card">
              <div class="stat-num success">{{ normalCount }}</div>
              <div class="stat-label">{{ t('system.profile.statusNormal') }}</div>
            </div>
          </el-col>
          <el-col v-if="disabledCount > 0" :span="8">
            <div class="stat-card">
              <div class="stat-num danger">{{ disabledCount }}</div>
              <div class="stat-label">{{ t('system.profile.statusDisabled') }}</div>
            </div>
          </el-col>
        </el-row>
        ...
        <div v-for="item in dataList" :key="item.dictCode" class="dict-item">
          <div class="dict-cell">
            <div class="dict-cell-key">{{ t('system.dict.labelKey') }}</div>
          ...
          <div class="dict-cell">
            <div class="dict-cell-key">{{ t('system.dict.valueKey') }}</div>
          ...
          <div class="dict-cell">
            ...
            <div class="dict-cell-val">
              <el-tag :type="item.status === '0' ? 'success' : 'danger'" size="small">
                {{ item.status === '0' ? t('system.profile.statusNormal') : t('system.profile.statusDisabled') }}
              </el-tag>
            </div>
          </div>
        </div>
