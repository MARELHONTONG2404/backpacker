<template>
  <div class="lock-container">
    ...
    <div class="lock-card">
      ...
      <div class="lock-hint">{{ t('pages.lockHint') }}</div>

      <div class="input-wrap" :class="{ shake: isShaking }">
        <input ref="passwordInput" v-model="password" type="password" :placeholder="t('pages.lockPlaceholder')" class="lock-input" @keydown.enter="handleUnlock" autocomplete="off" />
      ...
      <div class="lock-footer">
        <a href="javascript:;" @click="goLogin">{{ t('pages.lockRelogin') }}</a>
      </div>
    </div>
  </div>
</template>
...
const startClock = () => {
  const update = () => {
    ...
    currentDate.value = t('pages.dateFormat', {
      day: pad(now.getDate()),
      month: pad(now.getMonth() + 1),
      year: now.getFullYear(),
      weekday: t(`pages.weekday${now.getDay()}`)
    })
  }
  update()
...
const handleUnlock = async () => {
  if (!password.value) {
    showError(t('pages.lockPasswordRequired'))
    return
  }
  loading.value = true
  errorMsg.value = ''
