<template>
  <div id="tags-view-container" class="tags-view-container" :class="{ 'tags-view-container--chrome': tagsViewStyle === 'chrome' }">
    ...
    <el-dropdown class="tags-action-dropdown" trigger="click" placement="bottom-end" @command="handleDropdownCommand">
      ...
      <template #dropdown>
        <el-dropdown-menu class="tags-dropdown-menu">
          <el-dropdown-item v-if="!isAffix(selectedDropdownTag)" command="close"><close style="width: 1em; height: 1em;" />{{ t('layout.closeCurrent') }}</el-dropdown-item>
          <el-dropdown-item command="closeOthers"><circle-close style="width: 1em; height: 1em;" />{{ t('layout.closeOthers') }}</el-dropdown-item>
          <el-dropdown-item command="closeLeft" :disabled="isFirstView()"><back style="width: 1em; height: 1em;" />{{ t('layout.closeLeft') }}</el-dropdown-item>
          <el-dropdown-item command="closeRight" :disabled="isLastView()"><right style="width: 1em; height: 1em;" />{{ t('layout.closeRight') }}</el-dropdown-item>
          <el-dropdown-item command="closeAll"><circle-close style="width: 1em; height: 1em;" />{{ t('layout.closeAll') }}</el-dropdown-item>
          <el-dropdown-item command="fullscreen" divided>
            <template v-if="!isFullscreen"><full-screen style="width: 1em; height: 1em;" />{{ t('layout.fullscreen') }}</template>
            <template v-else><close style="width: 1em; height: 1em;" />{{ t('layout.exitFullscreen') }}</template>
          </el-dropdown-item>
        </el-dropdown-menu>
      </template>
    </el-dropdown>
    ...
    <!-- 刷新按钮 -->
    <span class="tags-action-btn tags-refresh-btn" :title="t('layout.refreshPage')" @click="refreshSelectedTag(selectedDropdownTag)">
      <el-icon><refresh-right/></el-icon> {{ t('layout.refresh') }}
    </span>

    <!-- 右键上下文菜单 -->
    <ul v-show="visible" :style="{ left: left + 'px', top: top + 'px' }" class="contextmenu">
      <li @click="refreshSelectedTag(selectedTag)"><refresh-right style="width: 1em; height: 1em;" />{{ t('layout.refreshPage') }}</li>
      <li v-if="!isAffix(selectedTag)" @click="closeSelectedTag(selectedTag)"><close style="width: 1em; height: 1em;" />{{ t('layout.closeCurrent') }}</li>
      <li @click="closeOthersTags"><circle-close style="width: 1em; height: 1em;" />{{ t('layout.closeOthers') }}</li>
      <li v-if="!isFirstView()" @click="closeLeftTags"><back style="width: 1em; height: 1em;" />{{ t('layout.closeLeft') }}</li>
      <li v-if="!isLastView()" @click="closeRightTags"><right style="width: 1em; height: 1em;" />{{ t('layout.closeRight') }}</li>
      <li @click="closeAllTags(selectedTag)"><circle-close style="width: 1em; height: 1em;" />{{ t('layout.closeAll') }}</li>
    </ul>
  </div>
</template>
...
import { translateMenuTitle } from '@/utils/routeI18n'
...
function getTagTitle(tag) {
  return translateMenuTitle(tag.path, tag.meta) || tag.title
}
