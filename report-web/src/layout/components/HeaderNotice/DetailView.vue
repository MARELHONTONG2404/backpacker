<template>
  <el-drawer v-model="visible" :title="t('navbar.noticeDetail')" direction="rtl" size="50%" append-to-body :before-close="handleClose" class="notice-detail-drawer">
      ...
      <div v-else class="notice-page">
        <div class="notice-type-wrap">
          <span v-if="detail.noticeType === '1'" class="notice-type-tag type-notify">
            <el-icon><Bell /></el-icon> {{ t('navbar.noticeTypeNotify') }}
          </span>
          <span v-else-if="detail.noticeType === '2'" class="notice-type-tag type-announce">
            <el-icon><Message /></el-icon> {{ t('navbar.noticeTypeAnnounce') }}
          </span>
          <span v-else class="notice-type-tag type-notify">
            <el-icon><Document /></el-icon> {{ t('navbar.message') }}
          </span>
        </div>
        ...
        <div class="notice-meta">
          ...
          <span class="meta-item">
            ...
            <span>{{ isStatusNormal ? t('navbar.statusNormal') : t('navbar.statusClosed') }}</span>
          </span>
        </div>
        ...
        <div class="notice-body">
          ...
          <div v-else class="notice-empty notice-empty--inner">
            <el-icon><Document /></el-icon> {{ t('navbar.noContent') }}
          </div>
        </div>
      </div>
    </div>
